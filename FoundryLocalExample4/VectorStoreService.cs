...
public class VectorStoreService
{
    ...
    public async Task InitializeAsync(int vectorSize = 512)
    {
        try
        {
            var info = await _client.GetCollectionInfoAsync(_collectionName);
            var existingSize = (int)info.Config.Params.VectorsConfig.Params.Size;
            if (existingSize != vectorSize)
            {
                await _client.DeleteCollectionAsync(_collectionName);
                throw new Exception("Dimension mismatch, recreating collection");
            }
        }
        catch
        {
